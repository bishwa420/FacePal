...
public class FaceDetection {
    ...
    /**
     * This method finds faces with given camera_id. It takes a
     * snap and detects faces. For successful detection only the
     * faces, cropped from the main snap, is returned.
     * This method can throw exception
     * if the needed_xml file is not found at the local C drive. Currently this
     * is under construction.
     *
     * @return
     * @throws Exception
     */
    public static IplImage[] detectFace(BufferedImage img) throws Exception {
        String classifierName = "C:\\needed_xml.xml";
        
        ImageIO.write(img, "jpg", new File("prev.jpg"));
        ...
        IplImage grabbedImage = toIplImage(img);
        Integer cnt = 0;
...
//        grabber.stop();
        ...
        System.out.println("cvcvt color is over");;
        //opencv_imgcodecs.cvSaveImage("birrhi"+(cnt++).toString()+".jpg", grayImage);

        //System.out.println("line 184");
        CvSeq faces = cvHaarDetectObjects(grayImage, classifier, storage,
                1.1, 3, CV_HAAR_FIND_BIGGEST_OBJECT | CV_HAAR_DO_ROUGH_SEARCH | CV_HAAR_DO_CANNY_PRUNING );
        
        
        int total = faces.total();
        System.out.println("detection is complete, total: " + total);
        ...
        */
        ...
        GrayConvertAndResize convert;
        for (int i = 0; i < faces.total(); i++) {
            CvRect r = new CvRect(cvGetSeqElem(faces, 0));
            int x = r.x(), y = r.y(), w = r.width(), h = r.height();
            cvRectangle(grabbedImage, cvPoint(x, y), cvPoint(x + w, y + h), CvScalar.RED, 1, CV_AA, 0);
            images[i] = toIplImage(cropImage(IplImageToBufferedImage(grabbedImage), x, y, new Rectangle(w, h)));
            opencv_imgcodecs.cvSaveImage("birrhis er aage"  + ".jpg", images[i]);
            convert = new GrayConvertAndResize(images[i]);
            images[i] = convert.grayConvertAndResize();
            opencv_imgcodecs.cvSaveImage("birrhis"  + ".jpg", images[i]);
        }
        cvClearMemStorage(storage);
    ...
    public static void main(String[] args){
        FaceDetection obj = new FaceDetection(0);
